    ...
    tabItems(
      ...
      tabItem(tabName = "forecasting",
              box(title = "Forecast Controls", width = 12, solidHeader = TRUE, status = "primary",
                  ...
                  # This tabsetPanel should be inside the box for correct layout
                  tabsetPanel(
                    id = "forecast_tabs",
...
server <- function(input, output, session) {
  # This reactive expression now references the globally loaded data frames
  ...
  observeEvent(input$platform, {
    # This observer also references the globally loaded data frames
    df <- switch(input$platform,
                 "TikTok" = tiktok_df_global,
                 "YouTube" = youtube_df_global,
                 "Instagram" = instagram_df_global
